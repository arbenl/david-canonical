...
# nixpacks.toml aptPkgs installs make/g++ into the nix profile during build,
# but we also run apt-get here as a fallback in case nix paths differ.
set -euo pipefail
...
echo "=== [2/3] Install CmdStan ${CMDSTAN_VERSION} ==="
...
# Register path with cmdstanpy so it doesn't search GitHub
uv run python -c "import cmdstanpy; cmdstanpy.set_cmdstan_path('${CMDSTAN_DIR}'); print('[david] CmdStan path set:', '${CMDSTAN_DIR}')"
...
print(f'[david] Binary compiled: {model.exe_file}')
